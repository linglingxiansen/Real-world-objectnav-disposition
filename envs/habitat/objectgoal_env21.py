...
points = {
    ...
    "left": '/api/joy_control?angular_velocity=1&linear_velocity=0.0',
    "right": '/api/joy_control?angular_velocity=-1&linear_velocity=0.0'
} # 线速度、角速度可自行更改
# direction = "forward"
...
class ObjectGoal_Env21(habitat.RLEnv):
    ...
    def reset(self):
        ...
        # if new_scene:
        obs = super().reset()
        obs['rgb'], obs['depth'], depth_colormap = self.camera.get_frame(self.rgb_mat,self.depth_mat)
        start_height = 0
        obs['rgb'] = obs['rgb'][:,:,::-1]
        self.scene = self._env.sim.semantic_annotations()
        ...
        self.last_sim_location = self.get_sim_location(pose['theta'],pose['x'],pose['y'])
        ...
        self.info['goal_cat_id'] = 0
        self.info['goal_name'] = 'plant'

        self.goal_name = 'plant'
    ...
    def step(self, action):
        ...
        if action=='FORWARD' or action==1:
            # for i in range(4):
            #     client.send(points['forward'].encode("utf-8"))
            #     data = client.recv(1024).decode()
            #     time.sleep(1)
            client.send(points['forward'].encode("utf-8"))
            data = client.recv(1024).decode()
            ...
            # for i in range(3):
            #     client.send(points['left'].encode("utf-8"))
            #     data = client.recv(1024).decode()
            #     time.sleep(1)
            client.send(points['left'].encode("utf-8"))
            data = client.recv(1024).decode()
            ...
            # for i in range(3):
            #     client.send(points['right'].encode("utf-8"))
            #     data = client.recv(1024).decode()
            #     time.sleep(1)
    ...
    def get_sim_location(self,theta,x,y):
    ...
    def get_pose_change(self):
        ...
        # print(results['current_pose'])
        pose = results['current_pose']
        curr_sim_pose = self.get_sim_location(pose['theta'],pose['x'],pose['y'])
