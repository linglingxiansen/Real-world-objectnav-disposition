...
class Sem_Exp_Env_Agent(ObjectGoal_Env21):
    ...
    def _preprocess_obs(self, obs, use_seg=True):
        ...
        if args.use_gtsem:
            ...
            sem_seg_pred[:,:,0][semantic_pred[:,:,0] == 1] = 1
            sem_seg_pred[:,:,1][semantic_pred[:,:,1] == 1] = 1
            sem_seg_pred[:,:,3][semantic_pred[:,:,3] == 1] = 1
        ...
        fn = '/home/lingfeng/L3MVN/tmp/dump/exp1/episodes/thread_{}/eps_{}/{}-{}-depth-{}.png'.format(
                self.rank, self.episode_no,
                self.rank, self.episode_no, self.timestep)
        cv2.imwrite(fn, depth)
        fn = '/home/lingfeng/L3MVN/tmp/dump/exp1/episodes/thread_{}/eps_{}/{}-{}-rgb-{}.png'.format(
                self.rank, self.episode_no,
                self.rank, self.episode_no, self.timestep)
    ...
    #     return depth
    def _preprocess_depth(self,depth, min_d=0.5, max_d=5):
        """预处理深度图像
        Args:
            depth: 输入的深度图像
            min_d: 最小深度值(米)
    ...
    def _visualize(self, inputs):
        ...
        rgb_vis = cv2.resize(self.rgb_vis,(640,360))
        self.vis_image[50:410, 15:655] = rgb_vis
        ...
        if args.print_images:
            fn = '{}/episodes/thread_{}/eps_{}/{}-{}-Vis-{}-{}.png'.format(
                dump_dir, self.rank, self.episode_no,
                self.rank, self.episode_no, self.timestep,inputs['pose'])
